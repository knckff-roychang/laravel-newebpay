...
use knckff_roychang\LaravelNewebpay\Library\NewebPay; //(model)取號完成
use knckff_roychang\LaravelNewebpay\Models\NewebpayCustomer; //(model)支付完成
...
class BackEndController extends Controller
{
    public function notifyRes(Request $request)
    {
        ...
        if ($data->Status === 'SUCCESS') {
            ...
            if ($data->Result->PaymentType !== 'CVSCOM') {
                $common['PayTime'] = $data->Result->PayTime;
            }
            ...
            switch ($data->Result->PaymentType) {
                ...
                case 'CREDIT': //信用卡 (包含 Google Pay、Samaung Pay)
                    ...
                    //若有紅利折抵 (才會傳參數)
                    if (isset($data->Result->RedAmt)) {
                        $insert['RedAmt'] = $data->Result->RedAmt;
                    }

                    //若有DCC動態貨幣轉換交易 (才會傳參數)
                    if (isset($data->Result->DCC_Amt)) {
                        $insert['DCC_Amt'] = $data->Result->DCC_Amt;
                    }

                    if (isset($data->Result->DCC_Rate)) {
                        $insert['DCC_Rate'] = $data->Result->DCC_Rate;
                    }

                    if (isset($data->Result->DCC_Markup)) {
                        $insert['DCC_Markup'] = $data->Result->DCC_Markup;
                    }

                    if (isset($data->Result->DCC_Currency)) {
                        $insert['DCC_Currency'] = $data->Result->DCC_Currency;
                    }

                    if (isset($data->Result->DCC_Currency_Code)) {
                        $insert['DCC_Currency_Code'] = $data->Result->DCC_Currency_Code;
                    }
    ...
    public function customerSuccess(Request $request)
    {
        ...
        return view(config('newebpay.customer_blade'), ['log' => $customer]);
    }
}
