...
class TestController extends Controller
{
    ...
    public function tradeButton(Request $request)
    {
        ...
        //可選參數
        if ($request->has('ReturnURL') && $request->ReturnURL !== null) {
            $data['ReturnURL'] = $request->ReturnURL;
        }

        if ($request->has('NotifyURL') && $request->NotifyURL !== null) {
            $data['NotifyURL'] = $request->NotifyURL;
        }

        if ($request->has('CustomerURL') && $request->CustomerURL !== null) {
            $data['CustomerURL'] = $request->CustomerURL;
        }

        if ($request->has('ClientBackURL') && $request->ClientBackURL !== null) {
            $data['ClientBackURL'] = $request->ClientBackURL;
        }

        if ($request->has('InstFlag') && $request->InstFlag !== null) {
            $data['InstFlag'] = $request->InstFlag;
        }

        if ($request->has('CREDIT') && $request->CREDIT !== null) {
            $data['CREDIT'] = $request->CREDIT;
        }

        if ($request->has('ExpireDate') && $request->ExpireDate !== null) {
    ...
    {
        if (trim(config('newebpay.env')) !== 'dev') {
            return abort(404);
        }
