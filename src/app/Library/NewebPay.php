...
class NewebPay
{
    ...
    {
        //傳來的資料陣列中，加入其他必要參數
        $tradeArr['MerchantID'] = config('newebpay.merchant_id'); //商店代號
        ...
        if (!array_key_exists('CustomerURL', $tradeArr)) {
            $tradeArr['NotifyURL'] = config('app.url') . '/api/newebpay/receive/notifyRes';
        }

        //如果沒自訂取號完成的 url，使用此套件設定好的路由
        if (!array_key_exists('CustomerURL', $tradeArr)) {
            $tradeArr['CustomerURL'] = config('app.url') . '/api/newebpay/receive/customerRes';
        }
        ...
        //回傳前端表單會用到的欄位
        return [
            'API' => (trim(config('newebpay.env')) === 'dev') ? self::TEST_API : self::API,
            'MerchantID' => config('newebpay.merchant_id'),
            'TradeInfo' => $aes,
            'TradeSha' => $sha256,
        ];
    }
    ...
    private static function createAES(array $data)
    {
        return self::create_mpg_aes_encrypt(
            $data, config('newebpay.hash_key'), config('newebpay.hash_iv')
        );
    }
    ...
    public static function decrypeAES($aes)
    {
        return self::create_aes_decrypt(
            $aes, config('newebpay.hash_key'), config('newebpay.hash_iv')
        );
    }
    ...
    {
        $str = 'HashKey=' . config('newebpay.hash_key') . '&' . $aes . '&HashIV=' . config('newebpay.hash_iv');
